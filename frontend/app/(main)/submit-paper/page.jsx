                  ...
                  {steps.map((step, index) => (
                    <div key={step.number} className="flex items-center">
                      <div className="flex flex-col items-center">
                        ...
                        <p className={`text-xs font-medium mt-1 text-center ${step.number === currentStep ? "text-red-600" : "text-gray-500"}`}>
                          {step.number}
                        </p>
                      </div>
                      {index < steps.length - 1 && (
                        <div className="flex items-center mx-2 sm:mx-3">
                          <div className={`h-0.5 w-12 sm:w-16 ${step.number < currentStep ? "bg-red-600" : "bg-gray-200"}`} />
                        </div>
                      )}
                    </div>
                  ))}
                  ...
                  {steps.map((step, index) => (
                    <div key={step.number} className="flex items-start">
                      <div className="flex flex-col items-center mr-4">
                        ...
                        {index < steps.length - 1 && (
                          <div className={`w-0.5 h-12 mt-2 ${step.number < currentStep ? "bg-red-600" : "bg-gray-200"}`} />
                        )}
                      </div>
                      <div className="flex-1 pt-2">
                        <p className={`text-base font-medium ${step.number === currentStep ? "text-red-600" : step.number < currentStep ? "text-gray-900" : "text-gray-500"}`}>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-4 sm:p-6 lg:p-8">
              ...
              {currentStep === 2 && (
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    <label htmlFor="articleTitle" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Article Title</label>
                    <input
                      type="text"
                      id="articleTitle"
                      name="articleTitle"
                  ...
                  <div>
                    <label htmlFor="keywords" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Keywords</label>
                    <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-3">Add up to 5 keywords relevant to your article.</p>
                    ...
                    {formData.keywords.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        {formData.keywords.map((keyword, index) => (
                          <span key={index} className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-red-600 text-white rounded-full text-xs sm:text-sm font-medium">
                            {keyword}
                            <button type="button" onClick={() => handleRemoveKeyword(index)} className="hover:text-gray-200 focus:outline-none">
